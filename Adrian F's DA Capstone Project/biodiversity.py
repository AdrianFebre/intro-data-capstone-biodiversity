...
counts = protection_counts.scientific_name

plt.bar(range(0,len(counts)),counts)

x_tic_spots = range(0,len(counts))
...
species['is_sheep'] = species.common_names.apply(lambda i:True if 'Sheep' in i else False)
species.head()
...
counts = obs_by_park.observations
...
BNP_weeks = num_sheep/250
print(BNP_weeks)


# In[72]:

YNP_weeks = num_sheep/507
print(YNP_weeks)
